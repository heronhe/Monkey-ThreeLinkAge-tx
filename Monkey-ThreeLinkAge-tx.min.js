...
e.oCity.length=1;if(e.oDistrict){e.oDistrict.length=1;}if(f==e.defaultText[0]){if(typeof e.updateProvice==="function"){e.updateProvice([]);}return;}for(var g=0;
g<e.counterParam[2][f].length;g++){e.oCity.appendChild(b(e.counterParam[2][f][g]));}if(typeof e.updateProvice==="function"){e.updateProvice(e.counterParam[2][f]);
}},false);}e.oCity.addEventListener("change",function(){var g=e.oCity.value;if(e.oDistrict){e.oDistrict.length=1;}if(g==e.defaultText[1]){if(typeof e.updateCity==="function"){e.updateCity([]);
}return;}if(e.oDistrict){for(var f=0;f<e.counterParam[3][g].length;f++){e.oDistrict.appendChild(b(e.counterParam[3][g][f].counterName,e.counterParam[3][g][f].id));
...
}}}if(typeof r.updateProvice==="function"){r.updateProvice(r.counterParam[2][o.p]);}}else{o.p="all";}var h;r.oCity.length=1;for(var f=0;f<r.counterParam[2][o.p].length;
...
}}}if(typeof r.updateCity==="function"){r.updateCity(r.counterParam[3][o.c]);}var p;if(r.oDistrict){r.oDistrict.length=1;for(var e=0;e<r.counterParam[3][o.c].length;
e++){m=document.createElement("option");p=r.counterParam[3][o.c][e];m.innerHTML=p.counterName;this.oDistrict.appendChild(m);if(r.locationStates){if(p.counterName==o.cn){m.selected=true;
}}}if(typeof r.updateDistrict==="function"){r.updateDistrict();}}if(typeof r.success==="function"){r.success({data:true});}},sortObJ:function(g,h){var l;
for(var f=0,k=g.length-1;f<k;f++){for(var e=0;e<k-f;e++){if(g[e][h]>g[e+1][h]){l=g[e+1];g[e+1]=g[e];g[e]=l;}}}return g;},setCounterId:function(m){var v=this,p=[],u=[],t={},w={},g,h,e,q;
for(var l=0,o=v.oProvice?m.length:1;l<o;l++){if(v.oProvice){e=m[l].name;u.push(e);t[e]=[];g=m[l].list;}else{g=m;t.all=[];}for(var k=0,s=g.length;k<s;k++){q=g[k].name;
w[q]=[];if(v.oProvice){t[e].push(q);}else{t.all.push(q);}h=g[k].list;for(var f=0,r=h.length;f<r;f++){p.push({id:h[f].counterId,p:e||null,c:q,cn:h[f].counterName,lc:h[f].location});
w[q].push({city:q,id:h[f].counterId,counterName:h[f].counterName,address:h[f].address||""});}}}return[p,u,t,w];}};a.prototype.constructor=a;return a;})();
