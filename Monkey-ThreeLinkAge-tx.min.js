...
this.counterParam=this.setCounterId(g.data);this.oProvice=g.provice||null;this.oCity=g.city;this.oDistrict=g.district;this.locationStates=g.location;this.success=g.success;
...
this.init();if(this.locationStates){if(c){e.getDistance(c);}else{if(!d){d=document.createElement("iframe");d.setAttribute("style","display:none");d.setAttribute("src","https://apis.map.qq.com/tools/geolocation?key=Q4EBZ-K2ORG-OSTQI-IKLRD-NTOXV-7DBVG&referer=pmlMap&effect=zoom");
...
}if(e.addDefaultText){if(e.oProvice){e.oProvice.appendChild(b(e.defaultText[0]));}e.oCity.appendChild(b(e.defaultText[1]));e.oDistrict.appendChild(b(e.defaultText[2]));
...
}}if(typeof e.updateCity==="function"){e.updateCity(e.counterParam[3][g]);}},false);e.oDistrict.addEventListener("change",function(){if(typeof e.updateDistrict==="function"){e.updateDistrict();
